...
import { TrackTO } from 'src/app/types/trackTO';
...
export class CollectionElementComponent {
  ...
  constructor(
    private router: Router, private backendCommunicationService: BackendCommunicationService, private musicPlayerService: MusicPlayerService
) {}
...
navigateTo(type: string, id: number) {
  ...
  this.album ? this.backendCommunicationService.getAlbum(id).subscribe((data) => (this.musicPlayerService.playTracks(data.trackTOList, data.cover_small))): this.backendCommunicationService.getPlaylist(id).subscribe((data) => (this.musicPlayerService.playTracks(data.trackTOList)));
}
